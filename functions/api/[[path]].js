...
function topic(message) { const text=message.toLowerCase(); if(/\b(css|flexbox|grid|stylesheet|estilo|seletor|box model|media query|responsiv)/.test(text)) return 'CSS'; if(/\b(javascript|js\b|dom|array|função|funcao|variável|variavel|objeto|evento|api|fetch|loop)/.test(text)) return 'JAVASCRIPT'; if(/\b(html|tag|elemento|semântica|semantica|atributo|formulário|formulario|head|body|h1|link|imagem|acessibilidade|site|página|pagina|web|landing page)/.test(text)) return 'HTML'; return null; }
...
export async function onRequest(context) {
  ...
  try {
    ...
    if(method==='POST'&&path.join('/')==='ai/chat') { const u=await auth(); if(u instanceof Response)return u; const message=String(body.message||'').trim(), permitted=allowed(u), t=topic(message); if(!message||message.length>4000)return json({error:'Envie uma pergunta de até 4.000 caracteres.'},400); if(!permitted.length)return json({error:'Seu acesso à IA Gabriela está bloqueado. Assine um plano para liberar o acesso.'},403); if(!t)return json({error:`A IA Gabriela responde somente dúvidas de ${permitted.join(', ')} para o seu plano.`},403); if(!permitted.includes(t))return json({error:u.plan==='BETA'?'Assine o Plano Pro para estudar CSS e JavaScript com a IA Gabriela.':`Seu plano libera somente ${permitted.join(', ')}.`},403); if(!env.AI)return json({error:'IA not configured.'},503); const history=(await env.DB.prepare('SELECT message,response FROM ai_conversations WHERE user_id=? ORDER BY id DESC LIMIT 8').bind(u.id).all()).results.reverse(); const betaInstruction=u.plan==='BETA'?' Você está atendendo uma pessoa do Plano Beta: entregue somente HTML, nunca gere CSS ou JavaScript. Em pedidos para criar um site, faça apenas a estrutura HTML e avise: "Você está usando o Plano Beta; vou ajudar somente na parte de HTML. Para CSS e JavaScript, assine o Plano Pro."':''; const messages=[{role:'system',content:`Você é Gabriela, professora de programação para iniciantes. Responda apenas sobre ${permitted.join(', ')}; ensine passo a passo em português.${betaInstruction}`},...history.flatMap(x=>[{role:'user',content:x.message},{role:'assistant',content:x.response}]),{role:'user',content:message}]; const data=await env.AI.run('@cf/meta/llama-3.2-3b-instruct',{messages,max_tokens:700,temperature:0.35}); const answer=String(data.response||data.result?.response||'I could not generate an answer now.'); await env.DB.prepare('INSERT INTO ai_conversations (user_id,message,response,user_plan) VALUES (?,?,?,?)').bind(u.id,message,answer,u.plan).run(); return json({answer}); }
