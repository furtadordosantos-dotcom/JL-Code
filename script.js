...
async function downloadCertificatePdf(selector,fileName,button,status){const certificate=document.querySelector(selector);if(!certificate)return;const originalLabel=button.textContent;button.disabled=true;button.textContent='Gerando PDF colorido…';try{await waitForImages(certificate);const canvas=await createCertificateCanvas(certificate);saveCertificateBlob(certificatePdfBlob(canvas),fileName);if(status)status.textContent='PDF horizontal, colorido e válido baixado com sucesso.'}catch(error){throw Error(`Não foi possível gerar o PDF: ${error.message}`)}finally{button.disabled=false;button.textContent=originalLabel}}
