...
loadFinalExam();
async function loadCertificate(){const status=document.querySelector('#certificate-status');if(!status)return;try{const state=await api('/api/final-exam/status');const c=state.certificate;if(!c){status.textContent='Você ainda não possui um certificado emitido.';return}const fullName=String(state.user.name||'').trim().replace(/\s+/g,' ');const nameParts=fullName.split(' ').filter(Boolean);const displayName=nameParts.length>1?`${nameParts[0]} ${nameParts.at(-1)}`:fullName;const validation=`${location.origin}/validar-certificado.html?code=${encodeURIComponent(c.certificate_code)}`;document.querySelector('#certificate-name').textContent=displayName;document.querySelector('#certificate-date').textContent=new Date(c.completed_at).toLocaleDateString('pt-BR');document.querySelector('#certificate-hours').textContent=`${c.training_days} dias de estudo`;document.querySelector('#certificate-score').textContent=`Aprovado (${c.score_percent}%)`;document.querySelector('#certificate-code').textContent=c.certificate_code;document.querySelector('#certificate-qr').src=`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(validation)}`;document.querySelector('#validate-certificate').href=validation;document.querySelector('#certificate-document').hidden=false;document.querySelector('#certificate-actions').hidden=false;status.textContent='Certificado emitido e protegido pela sua conta.';document.querySelector('#print-certificate').addEventListener('click',()=>window.print())}catch(e){status.textContent=e.message}}
loadCertificate();
