...
loadFinalExam();
async function loadCertificate(){const status=document.querySelector('#certificate-status');if(!status)return;try{const state=await api('/api/final-exam/status');const c=state.certificate;if(!c){status.textContent='Você ainda não possui um certificado emitido.';return}const fullName=String(state.user.name||'').trim().replace(/\s+/g,' ');const nameParts=fullName.split(' ').filter(Boolean);const displayName=nameParts.length>1?`${nameParts[0]} ${nameParts.at(-1)}`:fullName;const validation=`https://jlcode.pages.dev/validar-certificado.html?code=${encodeURIComponent(c.certificate_code)}`;document.querySelector('#certificate-name').textContent=displayName;document.querySelector('#certificate-date').textContent=new Date(c.completed_at).toLocaleDateString('pt-BR');document.querySelector('#certificate-hours').textContent=`${c.training_days} dias de estudo`;document.querySelector('#certificate-score').textContent=`Aprovado (${c.score_percent}%)`;document.querySelector('#certificate-code').textContent=c.certificate_code;document.querySelector('#certificate-qr').src=`https://api.qrserver.com/v1/create-qr-code/?size=240x240&data=${encodeURIComponent(validation)}`;document.querySelector('#certificate-qr-link').href=validation;document.querySelector('#validate-certificate').href=validation;document.querySelector('#certificate-document').hidden=false;document.querySelector('#certificate-actions').hidden=false;status.textContent='Certificado emitido e protegido pela sua conta.';document.querySelector('#print-certificate').addEventListener('click',()=>window.print())}catch(e){status.textContent=e.message}}
loadCertificate();
...
async function loadPublicCertificate(){const status=document.querySelector('#public-certificate-status');if(!status)return;const code=new URLSearchParams(location.search).get('code');if(!code){status.textContent='Informe um código de certificado válido.';return}try{const {certificate:c}=await api(`/api/certificates/${encodeURIComponent(code)}`),fullName=String(c.name||'').trim().replace(/\s+/g,' '),parts=fullName.split(' ').filter(Boolean),displayName=parts.length>1?`${parts[0]} ${parts.at(-1)}`:fullName,validation=`${location.origin}/validar-certificado.html?code=${encodeURIComponent(c.code)}`;document.querySelector('#public-certificate-name').textContent=displayName;document.querySelector('#public-certificate-course').textContent=c.course;document.querySelector('#public-certificate-level').textContent=`TRILHA PROFISSIONAL · ${c.level}`;document.querySelector('#public-certificate-date').textContent=new Date(c.completedAt).toLocaleDateString('pt-BR');document.querySelector('#public-certificate-hours').textContent=`${c.trainingDays} dias de estudo`;document.querySelector('#public-certificate-score').textContent=`Aprovado (${c.scorePercent}%)`;document.querySelector('#public-certificate-code').textContent=c.code;document.querySelector('#public-certificate-qr').src=`https://api.qrserver.com/v1/create-qr-code/?size=240x240&data=${encodeURIComponent(validation)}`;document.querySelector('#public-certificate-qr-link').href=validation;document.querySelector('#back-to-validation').href=validation;document.querySelector('#public-certificate-document').hidden=false;document.querySelector('#public-certificate-actions').hidden=false;status.textContent='Certificado verificado. Use o botão abaixo para salvar o PDF oficial.';document.querySelector('#download-public-certificate').addEventListener('click',()=>window.print())}catch(e){status.textContent=e.message}}
