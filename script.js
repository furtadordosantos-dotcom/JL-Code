...
async function downloadCertificatePdf(selector,fileName,button,status){const certificate=document.querySelector(selector);if(!certificate)return;const originalLabel=button.textContent;button.disabled=true;button.textContent='Gerando PDF colorido…';try{await waitForImages(certificate);let canvas;if(window.html2canvas&&window.jspdf?.jsPDF){try{canvas=await window.html2canvas(certificate,{backgroundColor:'#071425',scale:2,useCORS:true,allowTaint:false,logging:false,imageTimeout:15000,scrollX:0,scrollY:0,windowWidth:1280,windowHeight:820,onclone:(documentClone)=>{const clone=documentClone.querySelector(selector);if(!clone)return;clone.style.width='1120px';clone.style.minWidth='1120px';clone.style.maxWidth='1120px';clone.style.aspectRatio='16 / 9';clone.style.padding='34px 42px';clone.style.margin='0';const page=clone.closest('.certificate-page');if(page){page.style.width='1120px';page.style.minWidth='1120px';page.style.padding='0';page.style.margin='0';}}})}catch{/* a versão própria abaixo mantém o PDF disponível */}}if(!canvas)canvas=await createCertificateCanvas(certificate);saveCertificateBlob(certificatePdfBlob(canvas),fileName);if(status)status.textContent='PDF horizontal, colorido e válido baixado com sucesso.'}catch(error){throw Error(`Não foi possível gerar o PDF: ${error.message}`)}finally{button.disabled=false;button.textContent=originalLabel}}
